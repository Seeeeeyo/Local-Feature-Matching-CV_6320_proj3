...
class SecondMomentMatrixLayer(torch.nn.Module):
    ...
    def __init__(self, ksize: torch.Tensor = 7, sigma: torch.Tensor = 5):
        ...
        #######################################################################
        # TODO: YOUR CODE HERE                                                #
        base_kernel = get_gaussian_kernel(ksize, sigma).float()  # Convert to float32
        self.gaussian_kernel = torch.stack(
            [base_kernel, base_kernel, base_kernel], dim=0
        ).unsqueeze(1)
        ...
        self.conv.weight.data = self.gaussian_kernel.float()
        #######################################################################
...
class CornerResponseLayer(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        #######################################################################
        # TODO: YOUR CODE HERE                                                #
        Sxx, Syy, Sxy = x[:, 0, :, :], x[:, 1, :, :], x[:, 2, :, :]
        det_M = Sxx * Syy - Sxy * Sxy
        trace_M = Sxx + Syy
        R = det_M - self.alpha * (trace_M**2)
...
class NMSLayer(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = torch.where(x < threshold, torch.tensor(0.0, device=x.device), x)
        pool = nn.MaxPool2d(kernel_size=7, stride=1, padding=3)
        pooled = pool(x)
        output = torch.where(x == pooled, x, torch.tensor(0.0, device=x.device))
        #######################################################################
...
def get_interest_points(
    image: torch.Tensor, num_points: int = 4500
    ...
    ###########################################################################
    # TODO: YOUR CODE HERE                                                    #
    confidences, indices = torch.topk(R.view(-1), num_points)
    y, x = torch.div(indices, R.size(1), rounding_mode="floor"), indices % R.size(1)
...
def remove_border_vals(
    img, x: torch.Tensor, y: torch.Tensor, c: torch.Tensor
    ...
    mask = (
        (x >= border)
        & (x < img.shape[3] - border)
        & (y >= border)
        & (y < img.shape[2] - border)
    )
    x = x[mask]
    y = y[mask]
    c = c[mask]
